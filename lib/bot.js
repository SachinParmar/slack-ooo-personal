...
class Bot {
  ...
  constructor (config) {
    ...
    this.stats = {
      ...
      channels: new Set() // list of channels
    };
  }
  ...
  populateLookup (payload) {
    ['users', 'channels', 'groups', 'mpims', 'bots'].forEach((type) => {
      if (payload[type]) {
  ...
  /**
   * Announce setup to yourself
   *
   * @return {Bot} this
   */
  announceConfig () {
    const message = {
      text: 'Slack OoO Personal started with the following config:',
      channel: this.payload.self.id,
      attachments: [{
        fallack: 'OoO Bot Config',
        footer: `Slack OoO Personal ${PACKAGE.version} - ${PACKAGE.homepage}`,
        fields: [
          {
            title: 'Start Time',
            value: this.config.app.timebox.start
              ? moment(this.config.app.timebox.start).tz(this.payload.self.prefs.tz).format('h:mm:ss a Z, MMM Do YYYY') : 'now',
            short: true
          },
          {
            title: 'End Time',
            value: this.config.app.timebox.end
              ? moment(this.config.app.timebox.end).tz(this.payload.self.prefs.tz).format('h:mm:ss a Z, MMM Do YYYY') : 'never',
            short: true
          },
          {
            title: 'Message',
            value: this.config.app.message,
            short: false
          },
          {
            title: 'Respond to Direct Message?',
            value: this.config.app.respond.dm ? 'yes' : 'no',
            short: true
          },
          {
            title: 'Respond to Channel?',
            value: this.config.app.respond.channel ? 'yes' : 'no',
            short: true
          }
        ]
      }]
    ...
    if (this.config.app.respond.keywords.length) {
      message.attachments[0].fields.push({
        ...
        value: this.config.app.respond.keywords.join(', '),
        short: false
      });
    }

    if (this.config.app.announce.channels.length) {
      message.attachments[0].fields.push({
        title: 'Announce to Channels',
        value: this.config.app.announce.channels.join(', '),
        short: false
      });
    }

    if (this.config.app.announce.times.length) {
      message.attachments[0].fields.push({
        title: 'Announce Times (24 hour)',
        value: this.config.app.announce.times.join(', '),
        short: false
      });
    }
  ...
  handleMessage (message) {
    ...
    if (userName === `@${this.bot.identity.name}`) {
    ...
    } else if (user.is_bot && !this.config.app.respond.bot) {
      ...
      if ((message.event === 'direct_message') && this.config.app.respond.dm) {
        if ((this.responded.get(message.channel) || 0) < Date.now()) {
          ...
          response = {
            attachments: [
              {
                color: 'fa0029',
                fallback: this.config.app.message,
                pretext: this.config.app.response.dmprefix.replace('${user}', userName),
                text: this.config.app.message
              }
            ]
          };
          ...
          this.stats.users.add(message.user);
        }
        ...
        if ((this.responded.get(message.channel) || 0) < Date.now()) {
            ...
            attachments: [
              {
                color: 'BEDEAD',
                fallback: this.config.app.message,
                pretext: this.config.app.response.channelprefix.replace('${channel}', channelName),
                text: this.config.app.message,
                footer: `Slack OoO Personal ${PACKAGE.version} - ${PACKAGE.homepage}`
              }
            ]
          ...
          // update stats
          this.stats.responses++;
          this.stats.channels.add(message.channel);
        } else {
   ...
   */
  start () {
    this.controller.on('team_join,user_change,channel_created,bot_group_join,bot_channel_join', (bot, message) => {
    ...
    this.controller.on('rtm_close', () => {
      ...
      if (this.config.slack.autoReconnect && !this.manualClose) {
        this.connect();
  ...
  stop () {
    if (this.bot) {
      // Flag a manual close to avoid reconnect
      this.manualClose = true;

      // send closing message
      logger.info('Sending closing message');
      this.bot.say({
        text: 'Welcome back!\nSlack OoO Personal is signing off.',
        channel: this.payload.self.id,
        attachments: [
          {
            title: 'Statistics',
            text: 'Here is what I did while you were away:',
            footer: `Slack OoO Personal ${PACKAGE.version} - ${PACKAGE.homepage}`,
            fields: [
              {
                title: 'Total Responses',
                value: this.stats.responses
              },
              {
                title: 'Users Responded To:',
                value: Array.from(this.stats.users)
                        .map(userId => this.lookup.has(userId)
                          ? this.lookup.get(userId).profile.real_name : userId
                        )
                        .join('\n')
              },
              {
                title: 'Channels Responded On:',
                value: Array.from(this.stats.channels)
                        .map(channelId => this.lookup.has(channelId)
                          ? this.lookup.get(channelId).name : channelId
                        )
                        .join('\n')
              }
            ]
